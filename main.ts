...
namespace sdwireless {
    ...
    type EvtGamePos = (playerId: number, x: number, y: number) => void;
    ...
    let onGamePosition: EvtGamePos;  // 新增：游戏位置回调
    ...
    export function sdw_init(): void {
        ...
        irq.onEvent(PinEvent.PulseHigh, function () {
            ...
            // 游戏位置包解析
            if (msg && msg.length > 0) {
                // 检查是否是游戏位置包（第一个字节是0x50表示游戏位置）
                if (msg[0] == 0x50) {
                    // 格式：[0x50][playerId][x低][x高][y低][y高]
                    if (msg.length >= 6) {
                        let playerId = msg[1];
                        let x = (msg[3] << 8) | msg[2];
                        let y = (msg[5] << 8) | msg[4];
                        ...
                        if (onGamePosition) {
                            onGamePosition(playerId, x, y);
                        }
                    }
                }
                // 原有Microbit协议处理
                else if (onMbitNumber && msg[0] == PACKET_TYPE_NUMBER) {
                    let num = msg.getNumber(NumberFormat.Int32LE, 9)
                    onMbitNumber(num)
                }
                else if (onMbitString && msg[0] == PACKET_TYPE_STRING) {
                    let strLen: number = msg[9]
                    let strBuf = msg.slice(10, strLen)
                    onMbitString(strBuf.toString())
                }
                else if (onMbitValue && msg[0] == PACKET_TYPE_VALUE) {
    ...
    */
    ...
    // 游戏位置包类型
    const PACKET_TYPE_GAME_POSITION = 0x50;
    ...
    //% blockId=sdw_send_game_position block="Send Game Position x %x y %y player %playerId"
    //% weight=75
    //% playerId.min=0 playerId.max=3
    export function sdw_send_game_position(x: number, y: number, playerId: number): void {
        // 精简格式：[类型0x50][playerId][x低][x高][y低][y高] 共6字节
        let buf = pins.createBuffer(6);
        
        // 游戏位置包标识
        buf[0] = PACKET_TYPE_GAME_POSITION;
    ...
    //% blockId=sdw_on_game_position block="on Game Position"
    //% weight=71
    export function sdw_on_game_position(handler: (playerId: number, x: number, y: number) => void): void {
        onGamePosition = handler;
    }
